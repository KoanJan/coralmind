...
plan_example_json = """
{
  "nodes": [
    ...
    },
    {
      ...
      "output_names": {
        "target_market": "选择的目标细分市场（如“智能安防+智能照明一体化解决方案”）",
    ...
    },
    {
      ...
      "output_names": {
        "core_user_profile": "核心用户群体特征（如“26-35岁、新一线/二线城市、月入1-2万、自有住房的年轻夫妻，关注家居安全与生活便利性”）"
      },
      "is_final_node": false
    },
    {
      ...
      "output_names": {
        "product_positioning": "产品定位声明（如“为年轻家庭提供高性价比的智能安防+照明一体化解决方案，解决多设备联动与安装复杂问题”）",
        "differentiation_selling_points": "差异化卖点列表（如“统一APP控制所有设备、支持Matter协议跨品牌联动、15分钟快速安装”）"
      },
      "is_final_node": false
    },
    {
      ...
      "output_names": {
        "product_portfolio": "产品组合列表（如“核心产品：智能门锁、智能摄像头；扩展产品：智能灯泡、智能开关”）",
        "portfolio_priority": "产品优先级排序及理由（如“先做智能门锁（用户对安全性需求最高，竞品B公司在该领域有一定份额但性价比不足），再做智能摄像头（与门锁形成安防组合，提升用户粘性）”）"
      },
      "is_final_node": false
    },
    {
      ...
      "requirements": "结合用户调研中的价格接受度（如1000-2000元价格区间占比35%）、竞品的定价策略（如B公司中端定价1500元）及产品定位（如“中端高性价比”），制定产品价格区间（需覆盖单产品与套装）并说明理由",
      "output_names": {
        "price_ranges": "产品价格区间（如“智能门锁：800-1200元；智能摄像头：500-800元；安防+照明套装：1500-2500元”）",
        "pricing_reason": "定价理由（基于用户接受度、竞品定价、产品定位的综合分析，如“智能门锁定价800-1200元，低于A公司的2800元，高于D公司的600元，符合中端高性价比定位”）"
      },
      "is_final_node": false
    },
    {
      ...
      "requirements": "结合用户核心痛点（如“设备太多，APP太乱”）、期望功能（如“统一控制平台”）及市场技术趋势（如Matter协议），规划产品核心功能（需具体可落地）并说明每个功能解决的用户痛点",
      "output_names": {
        "core_features": "核心功能列表（如“统一控制APP、支持Matter协议联动、智能场景自动化、AI语音助手集成大模型、15分钟快速安装”）",
        "features_pain_solution": "每个核心功能解决的用户痛点（如“统一控制APP解决‘设备太多，APP太乱’的痛点；Matter协议联动解决‘不同品牌不能联动’的痛点”）"
      },
      "is_final_node": false
    },
    {
      ...
      "output_names": {
        "key_success_factors": "进入市场的关键成功要素列表（如“快速建立线上线下渠道覆盖、确保产品支持Matter协议联动、提供1对1安装指导服务”）",
        "risk_tips": "风险提示列表及应对建议（如“数据隐私问题：采用端到端加密技术；安装复杂度：开发智能引导式安装APP”）"
      },
      "is_final_node": false
    },
    {
